...
`getMyReactionIds()` / `getMyPortfolioReactions()` 는 아직 남아 있지만 **이제 부를 필요가 없습니다.**
`interview_qa` · `template` 처럼 목록 응답에 필드가 없는 대상에만 쓰세요.
...
이 문서의 스펙은 **유닛 833개 + E2E 396개 = 1,229개** 테스트로 검증돼 있습니다. 자세한 내용은 `TESTING.md`.
